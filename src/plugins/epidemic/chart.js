...
var config = {
    ...
    data: {
        ...
        datasets: [{
            ...
            label: 'Vaccinated',
            fill: 'end',
            backgroundColor: COLORS.circles.vaccinated.fill,
            borderColor: COLORS.circles.vaccinated.stroke,
            data: [],
            fill: '-1',
        }]
    },
...
function pushData(total_healthy, total_infected, total_in_intensive_care, total_cured, total_dead, total_vaccinated) {
    ...
    config.data.datasets[5].data.push(total_vaccinated);
    lineGraph.update();
}

function updateChart() {
    pushData(statistics.currentHealthy, statistics.currentInfected, statistics.currentInIntensiveCare, statistics.currentCured, statistics.currentDead, statistics.currentVaccinated);

    if (statistics.currentInfected === 0 && statistics.currentVaccinated === 0) {
        clearInterval(updateChartInterval);
    }
}