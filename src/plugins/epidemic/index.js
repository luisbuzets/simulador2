...
class Person {
        ...
        if (this.infected == false && this.immune == false) {
            ...
            if (this.needsIntensiveCare) {
                ...
                if (Math.random() < lethalityRate) {
                    ...
                    if(this.shouldVaccinateThisPerson()){
                        timers.push(setTimeout(function () {
                            this.vaccinate();
                            this.moveToStage('main');
                        }.bind(this), infectionDuration * 1000));
    ...
    update() {
     ...
     vaccinate(){
        if(this.infected == true && this.immune == false && this.vaccinated == false){
            this.vaccinated = true;
            this.circle.fill(COLORS.circles.vaccinated.fill);
            this.circle.stroke(COLORS.circles.vaccinated.stroke);

            let index = this.population.infectedPeople.indexOf(this);
            this.population.infectedPeople.splice(index, 1);

            statistics.currentVaccinated += 1;
            ...
            timers.push(setTimeout(function () {
                ...
                statistics.currentInfected = this.population.infectedPeople.length;
                statistics.currentVaccinated -= 1;
            }.bind(this), infectionDuration * 500));
    ...
    shouldVaccinateThisPerson() {
        // Determine the probability of vaccination (e.g., 10%)
        const vaccinationProbability = 23;
    
        // Generate a random number between 0 and 100
        const randomValue = Math.random() * 100;
    
        // Check if the random value is less than the vaccination probability
        return randomValue < vaccinationProbability;
    }
